  ...
  return (
    <>
      <Center>
        ...
        <TransitionGroup>
          {products?.map(({product, quantity,id})=>{
             return <CSSTransition classNames="item" key={id} timeout={500}>
              <Item
                quantity={quantity}
                product={product}
              />
        ...
        <div className={classes.total}>
          <div className="total_content">
            <h5>Total:</h5>
            <SwitchTransition>
              <CSSTransition timeout={500} classNames="fade" key={total}>
                <h5 className="total_price">{`$${total.toFixed(2)}`}</h5>
              </CSSTransition>
            </SwitchTransition>
